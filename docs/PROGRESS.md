...
- **Remaining:** Phase 2.5 validation run on host — opencode server, sessions, skills (17), plugins (18) all work. Render script fixed (opencode uses `type: "remote"` not `"streamable-http"`). Gateway plugin auth needs LL5_TOKEN provisioning. Phase 4.5 (standalone→compose transition), Phase 5 (deploy opencode), Phase 6 (switch and use).
