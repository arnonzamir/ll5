...
## 2026-09-10 (evening) — `agent.launch_loop` false alarm: deliberate relaunches no longer count

Arnon: "agent is relaunching in a loop." It wasn't: the alert fired at 19:37Z on 3 launches in 2 minutes and resolved itself at 19:44Z (max value 3, never more). The three were a container boot (19:34:08), a fresh relaunch 31s later (19:34:39) and an MCP-autoheal bounce (19:36:16). Cause of the stacking: I pushed ll5 (`65d6cb4`, touching `packages/ll5-run-shared/`) and ll5-run (`75cbb94`) a minute apart — both rebuild `ll5-run-claude`, so the orchestrator reprovisioned at 19:30:08 **and** 19:34:06, and the ll5 deploy rolled the MCP services → endpoint blip → bounce. The fresh relaunch 31s after every boot (seen on all four rolls today) is the **CI post-roll agent smoke** (DECISION-029 #4, `build-and-push.yml`: `docker exec … fresh-session; pkill claude`) — deliberate, but it runs from outside the image, so nothing inside explained it until the flag became self-identifying. The agent itself pinged a CRITICAL with a wrong cause (dev-channels picker) and corrected it 7 min later. **Fix (ll5-run `3aaee5a`):** a real loop is claude exiting on its own — `ll5-server` ends, the `unless-stopped` container restarts with the same writable layer, the next boot logs another line — whereas fresh/resume relaunches are asked for by the rate-limited watcher (15 min / 10 min) and cannot loop. `launches.log` lines are now `<ts> boot|fresh|resume` and the heartbeat's `launches_10m` counts boots only (untagged lines still count). The fresh flag is self-identifying: the watcher writes `watcher: <why>`, the CI smoke writes `ci-smoke: forced relaunch (DECISION-029)` (this commit), and `ll5-server` logs `[flag set <ts> by: <writer>]`. Verified on the 20:08Z roll: `boot` then `fresh` 32s later, flag named, alert quiet. The same roll's live e2e timed out on every MCP at once right after the deploy — re-run to separate host pressure from a regression.

---

## 2026-09-10 — File delivery: the agent can hand the user a file (web + Android + WhatsApp)

Arnon: "we need the agent to have a tool also to get the files and to deliver them to the user." The agent could already *find* stored media (`list_media` / `get_media_for`) but not deliver it — `send_whatsapp` was text-only (the Evolution client had no `sendMedia` at all) and `push_to_user` had no attachment field, so the only path was re-uploading a copy via `/chat/upload?public=1` and pasting a raw link into the text. Now: **awareness** gains `get_media(media_id)` → `{url, mime_type, filename, source, public, public_url}` (the absolute `public_url` only for a public file — the one an outside fetcher can actually read; `gatewayUrl` threaded into `registerMediaTools`). **messaging** gains `EvolutionClient.sendMedia()` (POST `/message/sendMedia/{instance}` with `media` as a URL — Evolution fetches it, no base64 round-trip) and the `send_whatsapp_media` tool (image/video/audio/document; caption REQUIRED because that is what carries the non-bypassable `[LL5]` prefix). Both send tools now share `utils/send-gates.ts` (`runWhatsAppSendGates`: prefix → account exists → connected → conversation `agent` permission → first-contact approval) so the text and media paths cannot drift apart on a safety gate. **gateway**: `POST /chat/messages` accepts `attachments` on an assistant-outbound message, validated by pure `utils/attachments.ts` (≤ 10, url required and limited to `/uploads/…`, `/public/…` or absolute http(s), `type` derived from the mime when absent) and stamped into `metadata.attachments` — no new columns, the same shape inbound attachments already use. **dashboard**: already rendered `metadata.attachments` on assistant bubbles, so images/file chips came free; fixed `uploadsUrl` (`/public/x` was collapsing onto the uploads path, absolute URLs were being mangled) and taught the `/api/uploads` proxy to pass a `public/` first segment through to the gateway's unauthenticated route without a bearer. **ll5-run channel**: `push_to_user` gains `attachments: [{url, filename, mime}]`, forwarded to the gateway. Persona: a "Sending a file to the user" section in the `media` skill — attach, never paste a bare URL; `/uploads` is fine for the LL5 thread (clients are logged in), WhatsApp needs a public URL. Tests: gateway `attachments.test.ts` (6), messaging `send-gates.test.ts` (7) + 3 `sendMedia` cases, dashboard 3 `uploadsUrl` cases; suites green (gateway 744, awareness 116, messaging 35). **Android rendering of outbound attachments is the open follow-up** — the data reaches it, the chat screen has to draw it.

---

## 2026-09-10 — `agent.daily_restart_missing` root cause: quote bug in session-start.sh

Arnon flagged the daily-restart alert. Root cause: the 2026-09-08T13:55Z delivery_policy deploy (DECISION-034 Phase 4) added an `f"..."` (double-quoted) Python f-string inside a bash `python3 -c "..."` block that is itself double-quoted — the unescaped `"` closed the bash string early, so `bash -n`/exec hit `line 94: syntax error near unexpected token '('` on **every** SessionStart:startup invocation from then on. Restarts kept firing fine (autoheal context-cap trigger unaffected — confirmed continuous in `mcp-autoheal-server.log`), but the hook itself (journal `session-restart` entry, user-model/GTD/lessons context block) silently died each time, so no journal entry landed for 3 days and the 26h `agent.daily_restart_missing` check correctly fired. Fix: escaped the f-string quotes (`f\"...\"`), verified clean against the container's actual bash 5.2.15 (not just local macOS bash, which parsed the broken version as valid — version-dependent lexer edge case). Pushed `ll5-run` `83b5b15`; hotfixed the live container file directly (old broken version backed up as `session-start.sh.broken-2026-09-08`) so the next restart doesn't wait on the deploy pipeline. Verified end-to-end: manual hook run exits 0, new `session-restart` journal doc confirmed in ES.

## 2026-09-08 (evening) — One notification entry point on the gateway; phone connector toggles retired (branch `feat/unify-notification-capture`, not pushed)

Arnon: the phone stops having per-connector capture; its generic notification capture is the only source and the gateway routes by package. **Gateway:** `processors/app-notification.ts` is now THE `app_notification` entry point — `routeAppNotification(es, pool, userId, item)` reads the two gates and applies the pure `decideNotificationRoute` table: catalog connector package + `data_sources.connector_<id>` on → handed to `processors/connector-event.ts` (unchanged) AND stored in `ll5_awareness_notifications` with `connector_id` (when `notifications_all` is on); connector off → a normal notification; IM package → dropped (the `message` path carries it); everything else → stored under `notifications_all`. The `notifications_all` gate applies only to the awareness store, never to connector routing; removals never reach the parser. `server.ts` lost the `app_notification` pre-routing and sourceMap key. Shared mapping gains `connector_id` keyword. **Phone sync:** `PUT /user-settings` with a `data_sources.connector_<id>` toggle now queues the existing `update_data_source` device command with `{ source: 'connector_<id>', enabled, packages: [catalog android_packages] }` (best-effort; SMS-only connectors produce none) so the app adds/drops the packages in include-list mode; `GET /me/notification-capture` (user bearer) returns `{ connector_packages: { '<id>': [pkg, ...] } for enabled connectors, im_packages: [...] }` for the app's include-list seed on start ("enabled" follows the gateway gate: never-toggled = enabled). Pure helpers in `utils/notification-capture.ts`. **Dashboard:** the `auth_type:'none'` hint reads "captured by the phone's notification capture — keep the app out of the exclusion list (include-list mode adds it automatically)". Tests: `__tests__/notification-capture.test.ts` (routing decision table, command payloads, seed) — gateway 738 tests / 62 files, `tsc` gateway + dashboard clean. Android worker: consume `packages` on `update_data_source` for `connector_*` sources, seed from the new route, drop the Connector capture screen.

## 2026-09-08 — All-app notifications + phone calls, server side (branch `feat/all-notifications-calls`, not pushed)

Arnon: "I want ll5 to be aware of all phone notifications from all apps (with inclusion and exclusion options) and phone calls." Awareness stores, the agent queries — no new trigger flood (ISS-033 / DECISION-031). **Gateway:** `app_notification` gains optional `app_label, category, channel_id, ongoing, key, when, removed`; routing by package in `processItem` — catalog connectors keep `connector-event.ts`, IM packages (`IM_PACKAGES` in `processors/app-notification.ts`: WhatsApp/Telegram/Signal/SMS/Slack/Gmail) are dropped because the `message` path already carries them, everything else goes to the new `processors/app-notification.ts` → `ll5_awareness_notifications` (one doc per Android notification `key`, else a content hash; an update re-posts on the same `_id`, `removed:true` stamps `removed_at`; no notable event, no system message by default). Per-package routing `user_settings.settings.notifications.routing { '<package>': immediate|batch|ignore }` (default batch; `ignore` = not stored): `immediate` = one `[Notification] <app>: <title> — <text>` system message under the connector cost-guard rules (3 per package per hour, overflow into one `GroupCoalescer` burst of 15 min / 12 items, then stored only until the next hour; ongoing notifications and removals are never immediate). Gate `data_sources.notifications_all`. 30-day retention: `pruneOldNotifications` on the heartbeat's new-day edge. New `phone_call` item (`state ringing|offhook|idle`, `direction`, `number`, `contact_name`, `started_at`, `ended_at`, `duration_s`, `call_log_id`) → `processors/phone-call.ts` + pure `utils/call-state.ts`: one `ll5_awareness_calls` doc per call (upsert key `started_at+number`, else `call_log_id`), `contact_name`/`person_id` resolved from `messaging_contacts` by phone number or WhatsApp JID when the phone sent no name, a `missed_call` notable event on idle with `direction:'missed'` (or ringing → idle with no offhook; severity medium when the number is a known person), and a `[Call] Missed call from …` system message only when that person's `contact_settings.routing` is immediate/agent. Gate `data_sources.phone_calls`. **Delivery mode** gains `on_call` (offhook, ≤ 3 h, no idle yet) between `driving` and `meeting`; `modeCeiling` treats it like meeting (push_notify). **Awareness MCP:** `query_notifications({ package?, app?, since?, until?, search?, include_ongoing?=false, limit≤100, cursor })` and `query_calls({ since?, until?, direction?, number?, contact?, limit≤100, cursor })` (repositories `notification.repository.ts` / `call.repository.ts`, shared result cap + cursor); `get_situation` adds `on_call {since, number, contact}|null`, `recent_missed_calls` (24 h, ≤ 5) and `recent_notifications` (2 h, non-ongoing, ≤ 10 lines `HH:MM app: title — text≤80`). Index mappings live in `@ll5/shared` `AWARENESS_INDICES` (both services ensure them at boot). Dashboard `/settings/data-sources` has the two toggles. Persona: one line under "Where Data Goes" (+301 B). Tests: gateway `app-notification.test.ts` (routing, dedupe/removal, cost guard, rendering) + `call-state.test.ts` (state machine, missed, stale reset, upsert key, `on_call` precedence), awareness `notifications-calls-tools.test.ts` (schemas, cap, cursor, situation line); e2e `it()` for both tools. Android worker: pending (builds against this contract).

## 2026-09-07 (23:30) — DECISION-034 Phases 2-4, agent side (branches `feat/delivery-persona-p2-4` here, `feat/delivery-agent-p2-4` in ll5-run-claude-code; not pushed)

Channel: every envelope now carries the seen model — `GET /me/seen-state` (60 s cache, same never-block shape as the delivery-mode probe; falls back to `user_seen_up_to` / `unseen_count` on the listen payload) is stamped as `unseen_count` (always), `user_seen_up_to` and `open_asks` (when informative), plus one hint line when something is unseen ("the user has not seen the last N messages — do not refer to them; restate, or hold"); 17 B when all is seen, under 200 B worst case. The activity-rail key is unified: `triggerIdOf(row)` = the scheduler event id (`metadata.event_id`, else the `[event_id: evt_…]` tag the gateway appends to every scheduler body), else the row id (= the trace id); it is stamped as `event_id` on the envelope, is the `currentTurn.trigger_id` that `narrate` rows carry, and now rides on `push_to_user` rows as `metadata.trigger_id`. Recorder: `eval_record.py` computes `trigger_id` the same way (`event_id` attribute, body tag, `id` attribute) and ships it in `SHIP_FIELDS` — `trace_id` stays local as before. `session-start.sh`: status line gains `unseen: N[, open asks: M]`; the user-model block renders `delivery_policy` as one line (bucket count, exploration rate, updated) instead of the JSON. Tests: 4 new `node --test` cases (42/42) for `seenEnvelope` / `triggerIdOf`, `test_trigger_id_rail_key` in the recorder suite, F5 whitelist test asserts `trigger_id` shipped and `trace_id` not; frozen block unchanged. Persona (+602 B, 64,347 → 64,949): "What the user has seen" paragraph (never assume the previous turn was read; `unseen_count`; restate the subject; digest/re-push is the machinery's job), one line in Delivery classes (modality comes from the learned `delivery_policy`; the agent declares class and stakes), the mirroring paragraph trimmed to pay for it. `consolidate` Step 6 "Delivery policy": `GET /me/delivery-stats?days=14` → per bucket `acted(m)` = max(acknowledged, done_by_deadline)/sent over modalities with sent ≥ 5, `preferred` = the weakest modality within 10 points of the best (else keep previous; never above push_alert), `floor` = one rung below (≥ chat), `score` = acted(preferred), dial-back one rung when too_much ≥ 3 and ≥ 3 × not_enough, exploration_rate −0.05 per pass floored at 0.10 and reset to 0.25 on a dial-back, written whole via `write_user_model(section:"delivery_policy")` capped at 60 buckets by n, `policy=<n>` on the tally. `daily` step 8: "open asks: N" on the brief's first line from `/me/seen-state`, only when N > 0. Not here: the gateway routes themselves (`/me/seen-state`, `/me/delivery-stats`, `/me/activity`, policy read at send time) and the Android reporters are the other worktrees; until the gateway lands, the channel's seen probe 404s harmlessly and the status line reads `unseen: n/a`.
## 2026-09-07 (22:00) — DECISION-034 Phases 2 + 4, dashboard side (branch `feat/delivery-dashboard-p2-4`, not pushed)

Chat: assistant/system rows with `metadata.rail === true` (the channel's stamp on proactive-turn narration) no longer render in the thread — `isRailRow` in `lib/chat/format.ts`, skipped in `buildRenderItems`; the dashboard tile's own fold and its "waiting for an answer" check skip them too; a user row is never hidden (`format.test.ts`). An "activity" toggle in the /chat header opens a right-hand rail (`components/chat/activity-rail.tsx`; collapsed by default, remembered in localStorage `ll5_chat_activity_rail_v1`) over `GET /me/activity`, newest first: time, trigger kind + summary, the one-line thought (folds at 160 chars, click to expand), decision chip (pinged / later / quiet, `deferral_ref` in the tooltip), reason, class, cost. "message" scrolls the thread to `#msg-<id>` (message-stream now wraps every bubble in that anchor) and flashes it; when the row is not loaded it pulls the open conversation's rows in a [trigger − 1 min, + 30 min] window through `/api/chat/messages?since&before` and retries, else says "not in the open thread". Tray items show as a chip until the dashboard tray (Phase 3). Poll 60 s while open, the visibility refresh throttled to the same interval, unmounted when closed, no badge. Gateway failure = amber line, last good list kept. Desktop sends no seen events (DECISION-034 Q5).
Settings: `/settings/delivery` "What works for you" (profile menu, under Notification Levels; not in the sensitive catalog): the learned policy from awareness `read_user_model` section `delivery_policy` as a table (bucket key split into class / stakes / deadline band / mode, preferred, floor, n, score; a one-line exploration-rate explanation), the 14-day `GET /me/delivery-stats` per bucket × modality as stacked CSS bars (acted-on = acknowledged + done_by_deadline over sent; dismissed and missed as the other segments; feedback tallies when present), and "Reset learning" behind a confirm dialog writing `{ version: 1, exploration_rate: 0.35, buckets: {} }` through `write_user_model`. Pure helpers + 8 tests in `delivery-types.ts`; policy and stats degrade to their own amber line. `npx tsc --noEmit -p packages/dashboard` clean; 12 new unit tests pass. Not verified against a live gateway: the two routes and the `rail` stamp are the gateway/channel tracks' deliverables.

## 2026-09-07 (22:00) — DECISION-034 Phases 2-4, gateway (branch `feat/delivery-gateway-p2-4`, not pushed)

Gateway side of the rail / seen model / learning / reach. **Phase 2 rail:** `GET /me/activity?since=&limit=(≤200)&cursor=` composes one entry per handled proactive trigger from `ll5_eval_moments` + rail chat rows (`metadata.rail=true` + `trigger_id`, the channel already stamps them) + `ll5_agent_journal` + `message_delivery` (+ `ll5_turn_costs` for `cost_usd`); pure composer in `utils/activity-rail.ts` (rule in the file header: key by `trigger_id`, a moment without one joins the nearest preceding trigger group within 30 min, the turn window runs to the next moment of the same session capped at 30 min, journal/classed message attach by window). Eval-moment whitelist + mapping gain `trigger_id`, `reason` (≤ 300 chars), `category`, `produced_message_id` (the live index takes them dynamically). **Phase 3 seen model:** migration 050 (`user_read_state(user_id pk, chat_seen_up_to, updated_at)`; `message_delivery` + `shown_at`/`opened_at`/`dismissed_at`/`feedback`/`explored`/`policy_bucket`); `POST /me/delivery-events {events:[{type, at, message_id?|tray_item_id?|delivery_id?, feedback?}]}` → `202 {accepted, rejected}` (idempotent COALESCE/GREATEST writes, read position monotonic, timestamps clamped to now; seen_at = first of notification_opened / tray_opened / read position ≥ created_at); `GET /me/seen-state` → `{chat_seen_up_to, seen_age, unseen_count, open_asks}`; the same four ride on `GET /me/delivery-mode` as `user_seen_up_to`/`user_seen_age`/`unseen_count`/`open_asks` (that is what the channel reads per trigger). unseen_count = assistant rows with `metadata.class` (or `notification_level`) after the read position, never rail rows, 7-day lookback when no position. Digest drops held do-bys with `seen_at`. **Phase 4 learning:** `GET /me/delivery-stats?days=14` → `{days, buckets:[{key:'<class>|<stakes>|<near|day|far>|<mode>', n, modalities:{<modality>:{sent, seen, acknowledged, done_by_deadline, dismissed, missed, too_much, not_enough, explored}}}]}` (`utils/delivery-stats.ts`, pure). Policy read at send time from the awareness MCP `read_user_model {section:'delivery_policy'}` with a minted user token, 5-min cache (`utils/delivery-policy.ts`); `pickModality`: bucket `preferred` (else Phase 1 map) → floored by bucket `floor` and by the agent's level → capped by delivery mode (sleep/quiet: push_silent, driving/meeting: push_notify, critical stakes: no cap) → with probability `exploration_rate` (policy's, default 0.35) one rung stronger; `explored` + `policy_bucket` recorded on the row; critical stakes keep the Phase 1 critical level. fyi is unchanged (agent level or chat). **Reach rung:** `utils/reach.ts` — self JID = `<digits(messaging_whatsapp_accounts.phone_number)>@s.whatsapp.net` of the connected account (fallback `user_settings.settings.reach.whatsapp_jid` + optional `reach.account_id`), sent through the messaging MCP `send_whatsapp {account_id, to, message, confirmed:true}` with a user token; text `[LL5] LL5: <subject> — <first line> (reach: unacknowledged, due HH:MM)` (the `[LL5]` prefix is the MCP's non-bypassable outbound gate); once per delivery via an atomic `reach_sent_at` claim on the escalation JSON, result recorded as `reach_result`; still gated by delivery mode, FCM `rung:'reach'` still sent. `utils/mcp-call.ts` is the shared server-side MCP call (server.ts now uses it). Tests: `delivery-seen`, `delivery-stats`, `delivery-policy` (seeded RNG), `activity-rail`, `reach` (+ eval-moment whitelist, digest unseen filter) — 59 files / 697 tests, tsc + compose-lint clean.

Known: the messaging MCP refuses `send_whatsapp` unless the self conversation's `contact_settings.permission='agent'` — the self-chat must be granted that once (see HANDOFF); until then reach logs `PERMISSION_DENIED` in `reach_result` and the FCM reach rung stands alone. Migration 050 is additive; before it runs the events route returns 202 with everything rejected and attachDelivery falls back to the 049 column set.

## 2026-09-07 (10:30) — DECISION-034 Phase 1, agent side (branches `feat/delivery-persona` here, `feat/delivery-channel` in ll5-run-claude-code; not pushed)

Channel (`channel/lib/delivery-contract.mjs`, pure, 38 `node --test` cases): `push_to_user` gains `class` / `subject` / `due_at` / `stakes` / `ack_required` / `escalation`; `checkDeliveryContract` refuses in the caps' path — no class on a proactive turn (fyi assumed on a user turn and on compact rows), needs-you/do-by without subject (≤ 40, must open the first 80 chars of the text: NFKC, niqqud/bidi stripped, case-folded), without due_at (ISO with offset, future, ≤ 14 d) or stakes, do-by with `ack_required:false` or `escalation:"none"`, needs-you with a ladder, fyi with due/ack/escalation; a do-by below critical in quiet hours is sent with a note that the ladder starts at quiet-hours end. The `delivery` block goes to the gateway; `tray_item_id` is echoed. A per-process `currentTurn` marker (set on every delivered envelope, reaction, lifecycle event and mirrored cli row; proactive = channel not web/android/cli, the eval recorder's definition) gates `reply(channel:"web")` on a proactive turn (allowed only when `reply_to_id` is a user-role row on a user-facing channel) and stamps `narrate` rows with `rail:true` + `trigger_id`. Limitation: the marker is the last envelope delivered, not the turn the model is provably in; CLI typing is seen only through the mirror hook's `cli` row. Hooks: `decide_mirror.py` no longer treats unknown raw text as CLI typing — a raw trigger mirrors only when `cli_typed_sha` (written by `cli-input-mirror.sh`, passed by `stop-mirror.sh`) matches; otherwise `gated (raw trigger, not a conversation)` (+5 tests, 32/32). Persona: act-by-default ladder reports in the rail; "Delivery classes" paragraph; Notifications says the machinery picks the level from class + stakes; Periodic Self-Check uses classes (CLAUDE.md 62,985 → 64,347 B, +1,362; frozen block unchanged, frozen-rule test passes). `notify` skill: class table first, level demoted to a floor; `situation-check` step 5: an action item for the user is a do-by with a due time, never a chat line. Not done here: the gateway worker (validation, tray asks, ladder) and Android are the other two worktrees.
## 2026-09-07 (09:20) — Delivery contract Phase 1, gateway side (branch `feat/delivery-gateway`)

`POST /chat/messages` takes a `delivery` block (`class` fyi / needs-you / do-by, `subject` ≤ 40 leading the text, `due_at` future ≤ 14 d, `stakes`, `ack_required`, `escalation`), re-validated in `utils/delivery-contract.ts` (400 `code:'delivery_contract'`; a proactive message without a class is refused — the agent channel must ship its `push_to_user` fields in the same roll). Migration 049: `message_delivery` (modality, delivery mode, status sent / acknowledged / done / expired / missed, ladder state) + `tray_items` ask columns (`subject`, `due_at`, `ack_required`, `message_id`, `escalation`, `future_text`, `acknowledged_at`, `done_at`; status check widened). Push level by stakes: low silent, medium notify, high alert, critical critical (critical only when the message is `notification_level:'critical'`, else downgraded to high with a note); FCM data gains `message_id`, `tray_item_id`, `class`, `rung`, `collapse`, `subject`, `due_at`. Tray: `ask` cards lead `GET /me/tray` soonest due first; `POST /me/tray/:id/ack` and `/done` (idempotent) stop the ladder and post `[Tray] acknowledged|done: <subject>`; `GET /me/delivery` lists open deliveries with rung state. `scheduler/delivery-escalation.ts` (5 min): standard ladder re-push at max(send+10 min, due−45) at `alert`, alarm at due−15, reach at due−5 (FCM `rung:'reach'` only — self-WhatsApp is Phase 2); one rung per tick, held in sleep / quiet hours unless critical; at due+30 min unacknowledged → missed / expired + `[Tray] missed:`. A do-by is never held: its row + tray card land at once, only its push waits for quiet hours to end (ladder start = max(now, quiet end)); held needs-you rows come back at release as real classed messages. Digest lines are untrimmed `HH:MM · class · subject — first line`, asks first, row metadata `kind:'quiet_hours_digest'`, `class:'fyi'`, `digest_of`. Tests: `delivery-contract.test.ts` (validator, ladder, nextRung, futureText tables), `delivery-escalation.test.ts` (tick over a fake pool), tray ask lifecycle in `tray.test.ts`, digest in `delivery-mode.test.ts`.

## 2026-09-08 (19:15) — One notification capture: live on both sides

Server `61c6a0c` rolled (run 34248890864): `app_notification` has one entry point — enabled connector packages go to the parsers and the awareness store (doc stamped `connector_id`), everything else to the store, IM packages dropped; `GET /me/notification-capture` answers with the enabled connectors' packages (max, isracard, bank, paybox, clalit, iec); `PUT /user-settings` on a connector emits `update_data_source connector_<id>` with `packages`. Android `f72b8be` installed on the phone 19:05 (Connector capture section, package map and toggles removed; connector apps out of the built-in exclusions; include-list seeding from the gateway; one-time migration of old toggles). Phone state: listener enabled, Phone/Call log/Contacts granted, capture mode default all-except — first notifications already stored (Calendar, 18:50). Next observable: a card push routed to both stores, the first call doc.

## 2026-09-08 (03:00) — Unifying notification capture (in progress)

Arnon: replace the finance connectors' phone-side capture with the generic capture. Two worktrees: gateway (one `app_notification` entry point routing by package: enabled connector → parsers + awareness store; other → awareness; `GET /me/notification-capture`; `update_data_source connector_<id>` carries `packages`) and Android (connector capture section, package map and toggles removed; connector packages leave the built-in exclusions; include-list seeding from the gateway; one-time migration of old toggles into include-list mode). DECISION-032 amended.

## 2026-09-08 (02:40) — All-app notifications and phone calls: live

Server merged `7b7a56c`, run 34234821788 green (every image, deploy, e2e incl. the two new contracts). Verified: awareness `tools/list` has `query_notifications` and `query_calls`; indices `ll5_awareness_notifications` / `ll5_awareness_calls` exist (0 docs — capture is off until Arnon sets the mode in the app and grants Phone + Call log); persona line present in the agent container; agent rolled 02:37. Android `b82f9a1` APK sent. The tray tap error is also closed: `3cbad3a` renders the ask in any status on ack/done.
...
Arnon: "I still get some 404 errors in the app when clicking on a needs you card. Not every time." Rows are intact (no ask was deleted; expired/done asks answer 200 idempotently), so the 404 means the id the app sent is not one of his tray rows. The gateway did not log 404s on the tray routes; it now logs the outcome, the raw param, whether the id exists under another kind/user, and the user agent, on both the ask and decision routes. Second screenshot (00:51 local, build `821f1e7`): "Network error — couldn't save it." on Done — the request never reached the gateway (no log line, VPN on): a phone-side transport failure. The earlier 404s are most plausibly taps during one of the day's eight gateway restarts, when the proxy answers 404 for a backend that is down. App fix: ask actions retry once after 1.5 s on IOException and on 404/502/503/504 (Android, APK sent). (First push `6726597` failed CI: the diagnostics used `req.get`, absent on the route-test fakes; fixed to a plain header read.)

## 2026-09-08 (00:25) — DECISION-034 Phases 2-4 live

Agent rolled 19:03Z (run 34154247123: channel `seenEnvelope`, persona "What the user has seen", consolidate Step 6, recorder `trigger_id`); gateway + dashboard rolled 19:18Z (run 34154630479): migration `050_delivery_seen.sql` applied; `GET /me/seen-state` answers (`unseen_count` 36, `open_asks` 2, no read position until the app reports); `GET /me/activity` composes entries (older moments have `trigger.id` null — new ones carry the id); `GET /me/delivery-stats` already shows real buckets (`do-by|high|day|meeting` missed 1, `do-by|medium|near|meeting` 2); `/settings/delivery` routes. Reach prerequisite done: the self WhatsApp contact is linked to the self person record and its contact rule is `permission=agent` (routing unchanged). Android `d031109` APK sent to Arnon (activity sheet, seen/notification/tray events, feedback). Remaining verification is time-gated: the first policy write at tonight's 02:00 pass (`policy=` on the tally), seen events once the new APK is installed and the app opened, a reach rung on a real unacknowledged do-by.

## 2026-09-07 (23:40) — Phases 2-4 merged; rolling agent first

All four merged: gateway `cc652d1` (activity rail route keyed by trigger id, migration 050 seen model, `POST /me/delivery-events`, `GET /me/seen-state`, `GET /me/delivery-stats`, policy-driven modality with exploration 0.35, reach rung via `send_whatsapp` to the account's own number, eval-moment whitelist gains `trigger_id`/`reason`/`category`/`produced_message_id`), persona `7dc3b57` + channel `fda56d7` (envelope `unseen_count`/`user_seen_up_to`/`open_asks` + hint, trigger id on narrate/push rows and on the eval record, consolidate Step 6 writes `delivery_policy`), dashboard `fb18466` (activity rail, rail rows hidden, `/settings/delivery`), Android `d031109` (activity sheet, seen/notification/tray events, ask feedback; APK staged). Main: gateway 697, dashboard 87, messaging 25, compose lint 10. Known gap from the gateway worker: the WhatsApp reach rung needs the self conversation to carry `permission='agent'` in contact settings; until then only the FCM reach fires and `reach_result` records `PERMISSION_DENIED`. Rollout: this push `[skip ci]`, agent repo push (roll), then dispatch gateway + dashboard.

## 2026-09-07 (22:30) — DECISION-034 Phases 2-4 started (Arnon: "Go")

Four worktrees against one contract: gateway (`GET /me/activity`, migration 050 `user_read_state` + seen columns, `POST /me/delivery-events`, `GET /me/seen-state`, `GET /me/delivery-stats`, policy-driven modality pick with exploration, reach rung via `send_whatsapp` to self); agent (envelope `user_seen_up_to`/`unseen_count`/`open_asks` + hint, `trigger_id` shipped on eval moments, consolidate Step 6 writes `delivery_policy`, persona "what the user has seen"); Android (rail rows hidden, Activity sheet, chat_seen / notification / tray events, ask feedback); dashboard (activity rail beside the chat, `/settings/delivery` "what works for you", reset learning). Rollout order again: agent first, then gateway, then dashboard.

## 2026-09-07 (22:00) — Three detector bugs the agent reported, fixed

From the agent's own degradation report (journal 20:50 local): the unanswered-thread rule fires on groups (ISS-039, now direct threads only), delivery mode calls a 4.5 h "SAVE THE DATE" placeholder a meeting (ISS-040, `isUserMeeting()` heuristics), and `read_messages` drops text for forwards and media (ISS-041, `messageText()`). Uploads persistence (ISS-037) verified: a probe file written in the container is on the host after the roll. Gateway 649 tests, messaging 25.

## 2026-09-07 (21:15) — Image store wiped on every roll (ISS-037); dead FCM tokens pruned (ISS-038)

The agent told Arnon "Image store is down" at 21:05: `/app/uploads` had no volume, so each of today's five gateway rolls deleted every photo and voice note; `inspect_image` then 404s on anything older than the last roll. Fixed with bind mounts under the Coolify service dir (created and chowned to the container's `app` uid). While there: the sender was hitting two dead FCM tokens on every push (404 NotRegistered); it now prunes them. The push path itself was fine — the phone re-registered a fresh token at 15:39Z after the APK install. This push rolls the gateway once more (the last wipe); files uploaded after it persist.

## 2026-09-07 (16:40) — First real asks on the phone; Done returned 400

Arnon's screenshot: two agent-created `needs-you` asks rendered as cards (Technoda mail, Shana Tova card) — the contract is producing real asks. "Done" failed with HTTP 400: the app posted the composite list id `ask:<uuid>` while the route takes the raw `ask.item_id`. Fixed on both sides: app uses `ask.item_id` (Android, new APK sent); gateway strips an `ask:` prefix so the earlier build also works. The app's version stamp showed `ff4da0c` for the worktree build (base commit), not `b547c16` — cosmetic.

## 2026-09-07 (09:50) — DECISION-034 Phase 1 live and verified

Rollout: agent rolled 06:24Z (run 34090392187; channel `checkDeliveryContract` + persona 64,347 B in the container), gateway rolled 06:32Z (run 34090881786; migration `049_message_delivery.sql` applied, `DeliveryEscalation` scheduler started, 5-min ticks). Live test with the agent token: a `do-by` "Delivery test (ignore)" with a 25-minute deadline → `message_delivery` row (modality `push_notify`, not held), tray ask with the honest ladder line ("re-push 09:44 · alarm 09:44 · reach 09:54"), initial push; re-push rung sent 06:47Z; `POST /me/tray/:id/ack` → `{status:"acknowledged", changed:true}` then `changed:false` on repeat; delivery `acknowledged`, ladder cleared, tray `acknowledged`, system message `[Tray] acknowledged: Delivery test (ignore)` reached the agent 06:48Z. Android `b547c16` (ask cards, notification actions, alarm rung, local do-by alarm, draft overlap fix) is pushed; APK staged, phone off USB. Gateway note for the app worker's DTO: an ask item is `{ id: "ask:<uuid>", kind, question, context, escalation.future_text, ask: { item_id, message_id, class, subject, due_at, stakes, ack_required, status, rung_sent } }` — the Android DTO must read the nested `ask` object (verify on first real use). Next: Phase 2 (mirror/rail split), Phase 3 (seen model), Phase 4 (learning); the make-or-break metric is asks acted on by deadline vs tray items per day.

## 2026-09-07 (11:40) — Phase 1 merged: gateway, channel/persona, Android

All three worktrees merged (gateway `feat/delivery-gateway`, persona `feat/delivery-persona`, channel `5d410ed` on agent main, Android `b547c16` pushed; APK staged for the phone). Rollout order flipped after the gateway worker's warning: the new gateway 400s a classless proactive push, so the agent rolls FIRST (its channel sends the `delivery` block, which the current gateway ignores), then the gateway. This push is `[skip ci]`; the agent repo push dispatches run-claude from this main; the gateway build is dispatched after the agent is verified. Verification for the card case follows the design's Phase 1 line.
...
## 2026-09-06 (19:10) — /finance page + sensitive-page catalog with password re-validation

Branch `feat/finance-page` (dashboard + connectors). **Sensitive pages:** `packages/dashboard/src/lib/sensitive.ts` is the catalog (`SENSITIVE_PATHS = ['/finance', '/settings/connectors']`, prefix match) plus the pure step-up cookie maths (Web Crypto, shared by the edge middleware, server actions and 9 unit tests): cookie `ll5_stepup` = `${exp}.${base64url(HMAC-SHA256(AUTH_SECRET, "${uid}:${exp}"))}`, TTL 15 min, httpOnly/lax/secure-in-prod. The middleware sends a session without a valid cookie for ITS uid to `/verify?next=<path>` (also right after a token refresh and on the stale-serve path); `/verify` (needs a session) takes email + password, calls the gateway `POST /auth/token` WITHOUT replacing the session cookie and only mints the step-up when the gateway's `user_id` equals the session uid (the token carries no email claim and no gateway route exposes one, so the email is typed, and the uid check is what binds the confirmation to the session). `lib/step-up.ts` `requireStepUp(next)` is called first thing in every `/finance` server action and every `/settings/connectors` server action, so a direct action call cannot load data without the step-up. Nav entries in the catalog carry a small lock glyph. **/finance** (`app/(user)/finance/`): accounts header from `list_connectors` (new `snapshot` field: allow-list projection of `config.accounts/connections/data_through/accounts_fetched_at`, `packages/connectors/src/snapshot.ts`, 3 tests; balances render "no balance reported" when the aggregator sends none, as Discount does today) with "Financy data through <date>", per-connection freshness and "Sync Financy now" (`sync_connector`); filters period 7/30/90 (default 30), connector, account last 4 (applied after the fetch — the tool has no such filter), kind charge/refund, merchant substring (`query_ledger merchant`); `query_ledger` pages of 100 with cursor "load more"; period summary computed server-side (`finance-summary.ts`, 5 tests) over up to 10 pages: charges, refunds, count, top 10 merchants, by `payload.category.main`, by account last 4 as CSS bar rows, other currencies kept out of the ILS bars; events panel = last 50 of `query_events` (OTP shows "one-time code", unknown shows connector + "unclassified" + notification title); open findings from `get_connector_digest(week)` with Resolve (`resolve_finding`); "Mark known" on a ledger row appends to `settings.connectors.rules.known_merchants` via `PUT /user-settings`. Amber banner when the MCP is down, ILS formatting with thousands separators, `dir="auto"` on merchant cells. Not verified against live data yet (worktree, no deploy): the exact `snapshot` contents on the deployed row, the digest's `open_findings` shape on a real finding, and the `/verify` round trip in a browser.
...
## 2026-09-06 (16:50) — Dashboard /settings/connectors is a picker (branch `feat/connectors-picker`)

`packages/dashboard/src/app/(user)/settings/connectors/`: `connectors-types.ts` now mirrors the shared catalog exactly (10 entries; `ConnectorAuthType` = `none | api_token | oauth`; `municipality`, `scraper_credentials`, `vault_browser_login` gone) plus dashboard-only `CONNECTOR_PICKER` (group + one-line "what it feeds") and `PICKER_GROUPS` (Cards and bank, Health, Utilities, Home, Ledger). `connectors-view.tsx`: picker at the top (checkbox cards grouped; select = `setConnectorEnabled(id, true)` = gateway `data_sources.connector_<id>` + `PUT /api/connectors/:id`, deselect = both off; state from `list_connectors.enabled`, gateway key as fallback under the amber banner); alert rules below it, shown only when a card/bank connector is selected; then one configuration card per selected connector (status, last sync/received, last error, schedule for ledgers) — credentials form only for `oauth` (client id / secret / user id + Financy hint) and `api_token` (token + base URL + Home Assistant hint), `auth_type:'none'` shows the "captured on the phone — Settings → Connector capture" line and no form, "Sync now" only for ledger connectors with stored credentials. Scraper credential form and per-card toggle removed. `connectors-server-actions.ts`: signatures unchanged; `submitConnectorCredentials` now refuses connectors whose catalog `auth_type` is not `api_token`/`oauth` or that mismatch the submitted type; label/kinds/auth_type always come from the catalog even if the service holds an older row. `npx tsc --noEmit -p packages/dashboard` clean.

## 2026-09-06 (16:20) — No scrapers; connectors UI becomes a picker

Arnon: "we need to allow selecting connectors, and only show configuration for selected ones. we won't scrape banks or anything fragile." Catalog: cards, bank (Discount, Leumi, OneZero), PayBox, Clalit, IEC, water are phone-event only (`auth_type:'none'`, no schedule); ledgers come only from Financy (`oauth`) and Home Assistant (`api_token`); `municipality` removed; `scraper_credentials` and `vault_browser_login` removed from the shared type and the MCP credential schema. DECISION-032 amended; design Section 4/9 scraper routes superseded. Dashboard rework (picker, config only for selected) in a worktree.

## 2026-09-06 (16:00) — Financy ledger connector (merged `42121b1`, rolling in run 34034788653)

First real ledger adapter. `financy` joins the catalog (`packages/shared/src/connectors/catalog.ts`: ledger only, `auth_type:'oauth'`, 360-minute cadence, financial) and the dashboard copy; the credentials form for `oauth` takes client id / client secret / user id (posted as `{ auth_type:'oauth', secret:{ client_id, client_secret, user_id } }`) with a one-line hint that they come from the Financy app, Settings → API. Adapter `packages/connectors/src/adapters/financy.ts` (registered by `registerBuiltinAdapters` in `registry.ts`): mints a token at `POST /oauth/token` `{ userId, clientId, clientSecret }`, caches it in memory per credential hash until the JWT `exp` minus 30 s (fallback `expiresIn`), re-mints once on 401 and throws `AdapterAuthError` on the second; a 403 whose body mentions the plan is `AdapterPlanError` → `pull_failed` with `status:'error', code:'plan_not_eligible'`. Reads only `GET /v2/data/accounts` (masked snapshot `{ id, providerId, accountType, currency, balances, last4 }` into the row's `config.accounts`, never `ownerInfo`) and `GET /v2/data/transactions` (`dateFrom = since - 3 days`, first pull today - 90 days, `sort=1`, `limit=200`, `nextPage` pagination); `/connections/refresh` (credits) and payments are never called. Sign convention verified on docs-financy.open-finance.ai/docs/transactions.md ("`amount.chargedAmount` ... Negative = debit"): negative → `charge`, positive → `refund`, amount stored absolute, `payload.direction` debit/credit; the transaction doc lists no `status` field, so `status` is passed through untouched; `isDuplicate` rows skipped; `external_id = id` (`id:SK` only when an id repeats in the batch); `occurred_at = transactionDate ?? valueDate`, `posted_at = bookingDate`; `account_ref` = last 4 of `debtorAccount.maskedPan` or of the account number; payload keeps category, type, installments, providerId, connectionId, original amount + currency, `foreign`. Cursor `{ since: max transaction date seen | today }`. Sync (`sync.ts`): `POST /api/sync { connector_id, scheduled:true }` is refused with `{ ok:false, reason:'not_due', retry_after_seconds }` until `last_success_at + (row.schedule_minutes ?? catalog default)`; manual `sync_connector` ignores the gate and keeps the 10-minute rate limit; an adapter's `config` patch is merged into the row after a successful pull. Reconciliation was scoped to one connector id, which could never link a Max app push to a Financy statement line: `reconcile.ts`, `openForReconcile`, `forReconcile` and `count` are now user-wide, and a pair matches on the same amount within ±3 days plus either the same `merchant_key` or the same masked account last 4 (merchant match outranks account-only; `matched_on` in the result). Gateway `scheduler/connector-sync.ts` (registered in `scheduler/index.ts`; knobs `settings.scheduler.connector_sync_minutes` default 15, `connector_sync_enabled`): posts `scheduled:true` for every catalog entry with a ledger feed and a default cadence via `connectors/client.ts` `postSync`, logs counts, raises `connector.<id>.sync` (warning) on any refusal other than not_due / disabled / no_credentials / no_adapter and clears it on the next success; an unreachable service fails the tick in the scheduler-health registry instead. Tests: connectors 62 (financy mapping fixture with charge / refund / installment / foreign / duplicate, two-page pagination, re-mint on 401, cursor advance, plan 403, due gate, config patch, cross-connector reconciliation through sync), gateway 585 (+6 scheduler), shared 138; `tsc --noEmit` clean for connectors / gateway / dashboard / shared; compose-lint 10 services OK. Not verified: a live pull against Financy (needs Arnon's credentials on the dashboard) and whether the accounts list returns `balances` as `{ amount, currency }` or `{ balanceType, balanceAmount }` (both documented, both handled).
...
## 2026-09-06 (14:10) — Dashboard /settings/connectors (connectors Phase 1)

New page `packages/dashboard/src/app/(user)/settings/connectors/` (page, `-types`, `-server-actions`, `-view`, copied from data-sources): a rules section at the top (`amount_threshold`, `duplicate_window_minutes`, `foreign`, `unknown_merchant`, `asleep_at_home`, `known_merchants`, saved under `user_settings.settings.connectors.rules` via `PUT /user-settings`), then one card per catalog entry with label, kinds, sensitivity, status badge, last sync, last error, an enable toggle, a credentials form keyed on `auth_type` (`scraper_credentials`: username/ID + password + optional card last 4; `api_token`: token + base URL; `vault_browser_login` / `none` / `oauth`: one line saying how it authenticates), a pull-schedule field and "Sync now" (disabled unless enabled, credentials stored and the MCP answered). Enabling writes the gateway kill switch `data_sources.connector_<id>.enabled` first (so `isSourceEnabled` works unchanged) and then `PUT /api/connectors/:id` on the connectors service; credentials go through a server action to `POST /api/connectors/:id/credentials` and the client only sees ok/error. Reads use `mcpCallJsonSafe("connectors", "list_connectors")`; when the service is down the page still renders the catalog with an amber warning, enabled state from the gateway settings, status "Not configured" and sync/schedule disabled. Catalog display fields are copied into `connectors-types.ts` because the dashboard image does not build `@ll5/shared`. Plumbing: `env.CONNECTORS_MCP_URL` (default `https://mcp-connectors.noninoni.click`), `connectors` in `lib/api.ts` MCP_URLS, nav entry after Data Sources, `CONNECTORS_MCP_URL: http://connectors:3000` on the dashboard compose service. `tsc --noEmit -p packages/dashboard` clean; compose-lint 9 services OK. Not yet deployed; the `connectors:` service is a separate branch.
## 2026-09-06 (14:20) — Connectors Phase 0: the `connectors` MCP (branch `feat/connectors-mcp`)

New package `packages/connectors` (PostgreSQL, health-MCP skeleton): `server.ts` (shared `tokenAuthMiddleware`, `runWithRequestContext`, `withToolLogging`, stateless `/mcp`, `/health`, migration runner on boot), migration `001_connectors.sql` with the five tables from the design (`connectors`, `connector_credentials`, `connector_events` UNIQUE `(user_id, dedupe_key)`, `connector_ledger_rows` UNIQUE `(user_id, connector_id, external_id)`, `connector_findings`), repository interfaces + PG implementations that take `user_id` from the request context only and encrypt payloads/secrets with the new shared AES-256-GCM helper (`packages/shared/src/encryption.ts`, moved from the identical google/health copies, which are untouched). `merchant_key` = HMAC-SHA256 of the normalized merchant with a sub-key derived from `ENCRYPTION_KEY` (`utils/keys.ts`). Pure `reconcile.ts` (same amount + same merchant_key within ±3 days, one-to-one, closest pair first). Adapter interface + registry with NO adapters: `sync_connector` returns `{ ok:false, reason:'no_adapter' }` (the retention/reconcile step still runs). Eight tools: `list_connectors`, `query_events`, `query_ledger` (`merchant` ILIKE over the decrypted page, cursor in raw-row offsets), `get_connector_digest` (`today|yesterday|week` in the user's zone), `resolve_finding`, `sync_connector` (1 per 10 min per connector, in memory), `ingest_ledger_rows` (strict Zod, ≤200 rows, `memo` ≤200 chars, upsert on `external_id`), `submit_otp` (60 s TTL, in memory). List results go through `capItems`/`pageFields`. New `withToolLogging(server, getUserId, { redactResults })` option in `@ll5/shared` — the three financial read tools log `{ redacted: true }` as their result in `ll5_audit_log`; every other MCP unchanged. REST on the same app behind the same bearer middleware: `POST /api/events` (gateway envelope, idempotent on `dedupe_key`, 201/200 + `ConnectorEventAck`), `PUT /api/connectors/:id`, `POST /api/connectors/:id/credentials` (204, never logged), `POST /api/sync`. Retention inside sync: `payload_enc` nulled after 90 days, open events `expired` after 48 h (a finding only when the connector has ledger rows), ledger 24 months (`config.retention_months`), resolved findings 12 months. Wiring: compose `connectors:` block (image `ll5-connectors`, `mcp-connectors.noninoni.click`, same DB as health), the three package lists in `build-and-push.yml`, `mcp-endpoints.json` `connectors`, an e2e `list_connectors` contract. Tests: 32 in connectors (reconciler, keys, ingest schema refusals, OTP TTL, digest ranges, sync refusal ladder against an in-memory fake) + shared encryption round-trip and redaction option; `tsc` clean for shared and connectors; compose-lint 10 services OK. Deviations from the health precedent: no Elasticsearch client (PG only; `ELASTICSEARCH_URL` is for audit/app-log), `foreign` column named `is_foreign` (reserved word), events carry `account_ref`. Not yet: gateway `POST /api/events` caller, dashboard page, adapters (Phases 1-3).
## 2026-09-06 (14:15) — Connectors Phase 1, gateway side (branch `feat/connectors-gateway`)

The phone-to-agent path for credit-card notifications. `types/push-data.ts` gains the `app_notification` item (`package, title, text, big_text, post_time`); `server.ts` routes it to NEW `processors/connector-event.ts`, gated by `isSourceEnabled(connector_<id>)` (fail-open like the other sources; a package outside the catalog is dropped before any storage). SMS `message` items whose sender matches `connectorForSmsSender()` take the same path IN ADDITION to their existing `ll5_awareness_messages` row. Pure parsers in `src/connectors/parsers/` (`isracard`, `max`, `cal`, `sms-generic` + shared `common.ts`): amount (Latin / Arabic-Indic / fullwidth digits, thousands separators, ₪ / ש"ח / שח / NIS / ILS / $ / USD / € / EUR / £), currency, foreign flag, merchant, card last 4, occurred_at (dd/mm + hh:mm in the post's own offset, year rolled back across New Year), kind charge / refund / bill / notice / otp / unknown, `dedupe_key = sha256(connector_id|package-or-sender|post_time|title|text)` over the RAW fields. OTP-looking texts become `kind:'otp'`, marketing pushes `kind:'unknown'` with amount null; both are stored, neither triggers. Fixtures: zenmoney/sms-formats Isracard/Amex (`הפועלים-il_15700`), Max (`max-il_15750`) and Leumi Card (`לאומי-il_15701`) samples, plus the Geektime Cal shape from the research note; Cal push wordings are synthetic and marked unverified in the test. Real templates read from the phone's SMS inbox (anonymized): Clalit appointment set / cancelled (`parsers/clalit.ts` → `kind:'appointment'`, `occurred_at` = the stated date+time in Asia/Jerusalem via `zonedIso`, payload `doctor` / `clinic` / `action`), prescription ready (`notice`, `subject:'prescription_ready'`), Clalit / OneZero / Cal-bit OTPs (`otp`, codes replaced by `<redacted>` in the payload), the MayanotH water bill (`water`, `bill`, `ע"ס 729.27` with no currency token = ILS). The cards on this phone send no per-transaction SMS, so charges will come as app pushes whose wording is not public: any catalog-package push with an amount pattern and no other recognizable shape is a `charge` (title = merchant candidate unless it is the app's brand name, `payload.merchant_source:'title'`), marketing wording still wins, anything else is `unknown` — raw title/text/big_text always land in the payload so Phase 1 collects real samples. Parser dispatch: cal / max / isracard / clalit have issuer files; `bank`, `paybox`, `water`, `iec` and any future catalog id use the generic parser (the catalog is imported from `@ll5/shared`, never edited here; the staleness-check test derives its expected keys from the catalog). Rules engine `src/connectors/rules.ts` (pure): `amount_threshold` (default 500), `unknown_merchant`, `foreign`, `duplicate` (same amount + currency + merchant key within 10 min), `asleep_at_home` (delivery mode `sleep` + location state `home`); thresholds and `known_merchants` from `user_settings.settings.connectors` (`connectors/settings.ts`, 60 s cache); known merchants otherwise = seen twice in 90 days in an in-memory per-process LRU (`merchant-memory.ts` — a restart forgets; documented). Trigger ladder in the processor: rule hit → `insertSystemMessage` (`[Card] 214 ILS at <merchant> 12:31 (card 4321) — unknown merchant`, scheduler event `connector_rule`, FCM high only for `asleep_at_home`) under the cost guard `connectors/cost-guard.ts` (pure `decide()`: `CONNECTOR_IMMEDIATE_MAX_PER_HOUR`, default 3, per user per connector per clock hour; overflow into one `GroupCoalescer` window keyed `${userId}:connector:${id}`, 15 min / 12 items, one burst message; after the burst, digest only until the next hour). Every parsed event → `writeNotableEvent` (`connector_finding` on a hit, else `connector_event`; summary = connector + kind + amount, never the merchant). Storage: `POST /api/events` on the connectors MCP (`connectors/client.ts`, env `CONNECTORS_MCP_URL` default `http://connectors:3000`, bearer = a user-scoped ll5 token minted from `AUTH_SECRET`); `created:false` = duplicate, stop. If the MCP is unreachable the event is logged at error, alert `connector.ingest` (warning) is raised, and the rules + ladder still run so the phone event is not lost; the alert clears on the next successful POST. Observability: `scheduler/anomaly-monitor.ts` derives `connector.<id>.events` staleness checks (48 h, `suppressedBy: ['channel.mirror']`) from `CONNECTOR_CATALOG` for phone-fed connectors with a package or SMS sender (cal, max, isracard, clalit, iec); age comes from the in-memory `connectors/liveness.ts` map — null after a restart means "unknown, do not fire" until the first event. `connector.fanout` (warning) when a connector passes 10 immediate messages in a UTC day. `/admin/health` gains `connectors.cost_guard`. Compose: `CONNECTORS_MCP_URL` on the gateway service (the `connectors:` service itself lands with the Phase 0 worktree). Tests: `connector-parsers` (32, fixture-driven, incl. negatives and dedupe stability), `connector-rules` (30, table), `connector-cost-guard` (10, pure decision + coalescer + liveness), `connector-staleness-checks` (3); gateway suite 567/567, `tsc --noEmit` clean, compose-lint 9 services OK. Not verified: a real Cal / Max / Isracard app push (only SMS samples exist publicly), the Android package ids, and the end-to-end POST against the connectors MCP (not built yet in this worktree).

## 2026-09-06 (13:00) — Connectors: build started (Arnon: "lets do it")

Shared contract first: `packages/shared/src/connectors/catalog.ts` (static catalog: cal, max, isracard, bank, clalit, iec, municipality, home-assistant, with Android packages, SMS senders, auth type, cadence, sensitivity) and `events.ts` (the gateway → connectors MCP event envelope: `POST /api/events`, `ConnectorEventInput` / `ConnectorEventAck`). Package ids verified on the Play Store by the Android worker (Isracard `com.isracard.hatavot` + business app, Clalit `clalit.android`, IEC `com.ewavemobile.electriccompany`; the first guesses for those three were wrong). Phone inventory 14:15: Clalit, Discount, Max, Isracard, Leumi, PayBox installed; no Cal, no IEC app; cards send no per-transaction SMS (charges come as app pushes); real SMS templates captured for Clalit (appointment set/cancelled, prescription ready, OTP), the water corporation bill (`MayanotH`, "ע\"ס 729.27") and OneZero OTP. Catalog: bank = Discount + Leumi + OneZero, new `paybox` and `water` entries; APK with those packages installed 14:16 (Android `ff4da0c`). DECISION-032 records the choices. Persona gained one "External accounts" line and the `daily` skill one digest step. Four parallel worktrees follow: connectors MCP skeleton (Phase 0), gateway ingest + parsers + rules + cost guard (Phase 1 server side), Android capture (Phase 1 phone side), dashboard `/settings/connectors`. Decisions taken on the design's open questions (mid-level, standing authorization): OTP by chat paste first; connector tool results redacted from the audit log; retention 24 months ledger / 90 days raw text; weekly agent-driven portal pulls; Home Assistant entities decided in Phase 2; family card attribution later (card last 4 is captured).

## 2026-09-06 (12:30) — Connectors: generic design proposed

Arnon wants connectors for kupat holim, bank, credit cards, municipality, bills and home sensors, with per-transaction events, not only batches. A design subagent surveyed what exists and wrote `docs/design/connectors.md` (proposed): a `connectors` MCP (PG, copied from the health MCP skeleton) owning ledgers, events, findings and credentials with application-level AES-256-GCM; the gateway keeps ingest, pure parsers for phone notifications, a rules engine, the cost guard (3 immediate triggers per connector per hour, overflow through `GroupCoalescer`, rest to the morning brief via one line in the `daily` skill), notable events and the sync scheduler; eight read-only tools; a `/settings/connectors` page copied from `settings/data-sources`; Android sends `app_notification` items with `bigText` for catalog packages. Phase 1 = credit-card notifications end to end. Correction to the memory notes: the data-source config page IS built (`utils/data-source-config.ts`, dashboard page). A parallel research note on Israeli bank/card APIs, `israeli-bank-scrapers`, Home Assistant integrations (IEC etc.), Clalit and municipal portals landed: `docs/research/2026-09-06-israeli-connectors.md` (open banking is licensed-TPP only; `israeli-bank-scrapers` v6.10.0 is the practical route; Cal pushes every transaction, Max/Isracard SMS; IEC/water/Supergas/BOI rates have Home Assistant integrations; Clalit and city4u are scrape-only).
...
Merge review: the `processed` flag on `ll5_awareness_messages` is now set at flush time, not at push time, so a window lost to a restart or a failed insert is still reported by the next batch review.
...
The same watcher policy turned out to be the cost driver since the Opus 5 roll: a fresh session sits at ~120K tokens after its first trigger, so the 150K cap was crossed within minutes and the 1-hour anti-flap gap let sessions grow to 300–550K before restarting; every assistant message re-read that context. `ll5_turn_costs`: $85 on 09-04 (pre-upgrade), $239 on 09-05, $137 by 10:00 on 09-06 with $72 in a single hour. Cache reads are 57% of spend.

Shipped (agent `6ec517a`, rolled via run 34018053651): subagent-aware idle check, no cap restarts 02:00–03:30 local, cap 200K / gap 15 min (≈4 triggers per session at ~160K average). The reviewer's verdict and the levers left for Arnon (start-pack `active_context` 46 KB, persona 97 KB, `record_moment` per trigger, coalescing group-chat triggers) are in `docs/ISSUES.md` ISS-033. Third checkpoint scheduled 2026-09-07 03:12 in this session (session-only — if the laptop sleeps, run the runbook by hand).

Checkpoint items verified by hand this morning: hooks firing on 2.1.260 (session-save, context-pack, turn-cost all written 06:52Z), `isCompactSummary` 0 across all transcripts, every turn on `claude-opus-5`, bridge liveness live (`last_event_at` 06:53Z, `chats.update`), no `channel.whatsapp` alert overnight (only `throughput.inbound_messages`, resolved 23:52Z), narrative loop on Sonnet 5 refreshing/creating all night, DECISION-030 caps refusing (`NOT SENT — 228 chars > 200 cap`, then resent), quiet hours held nothing because the agent sent nothing proactive (journal: "Nothing sent: delivery_mode sleep"), `GET /me/delivery-mode` answering (`meeting`, calendar + illness).
...
## 2026-09-05 (22:30) — ISS-032: Topics alive again — observations link to the narratives they belong to

"Topics should be alive?" Yes, and they weren't: today's 50 observation subjects were fresh slugs (`arnon-saturday-mornings-with-kids`, `megalim-mudpasim-wine-tasting`) while the matching narratives exist under other slugs (`arnon-saturday-home-dad-mode`), so `list_narrative_work` saw refresh 0 and 25 one-off creates below the deliberate threshold. Fixed where it cannot be forgotten: `note_observation` (personal-knowledge) now finds the active topic narrative sharing ≥2 significant words with a new slug and tags the observation with that subject too, answering `linked` + the canonical ref (`subject-link.ts`, 4 tests); the session-start pack prints `[topic: ref]` next to each narrative; the persona says reuse before you invent. The loop's threshold stays at 2 (a narrative on the second observation). Deployed 15:35Z (run 33974951120: personal-knowledge + run-claude, smoke OK); expect `[note_observation] linked` in the knowledge MCP log and refreshes in `narrative-loop.log` within a day — the 03:12 checkpoint checks.

## 2026-09-05 (21:00) — Tickler alert reviewed (ISS-030); Android draft cards

Arnon asked whether `[Tickler Alert]` was a leftover debugging tool. It is the delivery path for `create_tickler` reminders and the agent's `kind: instruction` self-scheduled reviews — core, not debug — but its per-day dedupe lived in memory and every gateway deploy re-announced the same daily health ticklers (40 alerts in 7 days, 26/29 suppressed by the agent, twice in 22 minutes today). Persisted in `tickler_alerts_sent` (migration 048), written before send. Android: `DraftCard.kt` renders `[[draft to= via=]]` blocks as Copy / "Copy & open WhatsApp" (ACTION_SEND into com.whatsapp, chooser fallback); `AssistantContent` wraps `MarkdownBody` in `ChatScreen`; debug APK built from `feat/phone-activity-awareness` (the branch the installed app runs from) and handed to Arnon. Slack warnings (ISS-031): per-app silence checks on the mirror channels replaced by one `channel.mirror` check on `source=phone` at 24h — Slack is silent for whole days; the listener as a whole never was for more than 16.8h. Arnon then saw an "inbound volume dropped" banner (6 vs 65 median, Sat 16:47): a quiet afternoon — the only new Evolution messages were emoji reactions — so `throughput.inbound_messages` now stands down while the bridge shows an event within 30 min (`skipIf`), and `GET /me/bridge-liveness` exposes the signal. Arnon asked what updates the Topics list: the narrative loop had been gated at `promote_threshold: 2` while 25 one-observation subjects waited (ISS-032) — the gate briefly went to 1, spawned a worker that found nothing under the prompt's own threshold of 2, and was put back — the design is "a narrative on the second observation"; subject reuse by the agent is the real fix (ISS-032). The banner itself outlived its cause because `AnomalyMonitor` cleared only alerts in its in-memory `active` set — anything raised before a gateway restart stayed firing; it now clears from the DB's firing set too — and, since bridge liveness is in-memory, the check also stands down for the first 15 minutes after a gateway start instead of judging on "no evidence" (the 14:13Z deploy re-raised it that way).
...
## 2026-09-05 (17:30) — DECISION-030: assertive proactivity + concentrated style, enforced at the boundary

Data (7 days): 217 agent messages to 40 from Arnon, 68% agent-initiated, medians 150–435 chars, rituals 1.5–1.9K with headings and bold, 19 pushes between 01:00 and 05:00, hollow deferrals, 4 closed loops. The persona had said "concentrated" since August — no effect. Shipped: message caps enforced by the channel tool (`notice` 200 / `reply` 400 / `brief` 600 & 3 items / `detail` 1200, no headings or bold outside detail; refused messages are rewritten); gateway quiet hours (proactive non-critical pushes held in `held_messages` during sleep/23:30–06:30 and released as one 06:30 digest by `QuietHoursReleaseScheduler`; alerts skip the phone push at night); delivery mode (`utils/delivery-mode.ts`, `GET /me/delivery-mode`: sleep > quiet_hours > driving > meeting > sick > normal) stamped on every inbound envelope by the channel MCP; persona act-by-default ladder, one question with a default, mirror the user's register, `[[draft to= via=]]` blocks the dashboard renders as Copy / "Copy & open WhatsApp" cards (`draft-card.tsx`); baseline script row "Message shape". Tests: delivery-mode (quiet-hours math, precedence, sick detection, digest). Order of ship: ll5 pushed `[skip ci]` → dispatch gateway+dashboard → agent repo push (channel tool) → run-claude roll. Next batch: DECISION-028 #5 + ISS-004 structural deferral.

## 2026-09-05 (15:30) — ISS-013: the WhatsApp "bridge stall" was the monitor, not the bridge

14 `channel.whatsapp` criticals in 14 days, all between 01:07 and 05:21 local, all "no inbound for 2h" — while 01:00–05:00 is empty on most nights and Evolution had no fault (267 notifications lifetime for nothing). The flow monitor now has ground truth: every Evolution event that reaches `dispatchEvolutionEvent` is recorded (`utils/whatsapp-bridge-liveness.ts`), including `connection.update` state, and the decision is a pure, tested policy (`evaluateWhatsAppFlow`): socket closed > 5 min → stale any hour; quiet hour (empty on ≥5 of the last 14 same-hours) → silence is normal; daytime 2h silence → stale (the real anomaly, kept); daytime divergence → stale unless the bridge showed an event within 30 min. Alert text carries the bridge-event age and no longer suggests a restart first. Verification is tonight: no raise around 02:00; the 03:12 checkpoint checks it.

## 2026-09-05 (14:00) — DECISION-029: test purge, live contract tests, CI gating

Arnon's question after ISS-028 — "shouldn't a unit test have caught it?" — has the answer no: all three of the day's defects sat at boundaries unit tests cannot see (an external API's response shape, a TUI default of an upgraded binary, a compose env var), and CI ran no tests at all. Decided and shipped: 55 mock-assertion / review-batch test files deleted (1,730 → 1,057 tests; every remaining suite green), `packages/e2e` with live read-only MCP contract tests (5/5 against prod, including `read_messages` on a real WhatsApp conversation) and a compose lint (9 services OK), a post-roll agent smoke (boot + forced relaunch) in the deploy job, and CI gating: `unit-tests` (+ compose lint) → `build` → `deploy` → `e2e`. Full reasoning in `docs/decisions/DECISION-029-test-strategy-fewer-units-live-contracts.md`. The `e2e` job needs the `LL5_E2E_TOKEN` repo secret (set). First gated run failed exactly as designed — on `ll5-auth`, whose test script had been committed without its (untracked) tests: those tests are now committed and the loop runs `vitest --passWithNoTests` per package.
...
## 2026-09-04 (night) — ISS-012: the user-model history index died on the 1000-field limit

The "history indices take no writes" row was a silent outage, not archive-only. `ll5_agent_user_model_history` was never declared in `packages/awareness/src/setup/indices.ts`, so ES dynamic-mapped every `content.<person>.<key>` path of every snapshot until it hit the default `index.mapping.total_fields.limit` of 1,000 — the mapping has exactly 1,000 fields — and the last version that landed is 2026-06-20T22:34Z. Every nightly `write_user_model` since (Jul 31, Aug 37, Sep 3 calls) overwrote `active_context` without a snapshot because the history write threw inside a bare `catch {}` in `tools/journal.ts`. Fixed in three places: (1) the index is declared with the live index's mapping (`content` `enabled: false` — it is only ever read by id or listed by `user_id`/`section`/`archived_at`) and the live index was recreated in place via a count-verified temp copy (148 versions kept); (2) `write_user_model` distinguishes a 404 (first write) from a real failure, logs `user_model_history_snapshot_failed`, returns `warning` in the tool result, and preserves `created_at` across overwrites (it used to reset it every night); (3) `tools/lessons.ts` `snapshot()` logs non-404 failures, and the `learned_notes` append aborts on a non-404 GET failure instead of overwriting every learned note with one. Regression test in `__tests__/tools.test.ts`; awareness 230/230. Verify tomorrow: one `ll5_agent_user_model_history` doc with `archived_at` after tonight's 23:00 write.
...
## 2026-09-04 — Reconcile subsystem retired (DECISION-028 #1, ISS-008/009)

The DECISION-025 D3–D6 reconcile machinery never had an input (`gtd_horizons.conversation_id` NULL on all 493 h0 rows; 5,706 all-zero `ll5_reconcile_metrics` docs; 0 `reconcile_loop` calls), so it is removed from the ll5 repo rather than fixed. Deleted: gateway `src/reconcile.ts`, `src/reconcile-gate.ts`, `src/scheduler/reconcile-governor.ts` and the 5 tests `__tests__/reconcile{,-gate,-governor,-atomicity,-confirm}.test.ts`; gtd `src/tools/reconcile.ts` and `__tests__/reconcile{,-gate,-tools}.test.ts`; `packages/ll5-run-shared/prompts/reconcile-loop.md`. Edited: `scheduler/anomaly-monitor.ts` (the five checks `loop.reconcile_worker`, `loop.reconcile_governor`, `reconcile.missed_close_elevated`, `reconcile.wrong_close`, `reconcile.low_coverage`, their constants, and the `latestGauge` check kind + `latestDoc`/`runLatestGauge`/`cmp` helpers — nothing else used them; `percentileRegression` stays; 17 → 12 checks); `scheduler/index.ts` (`ReconcileGovernorScheduler` registration + `reconcile_governor_minutes` setting); `tray.ts` (`reconcile_confirm` kind, `enqueueReconcileConfirm`, `collectReconcileConfirmItems`, `POST /me/reconcile/confirm`); `server.ts` (`ll5_reconcile_metrics` mapping in `GATEWAY_INFRA_INDICES`); gtd `tools/index.ts` + `server.ts` (the `list_reconcile_work`/`reconcile_loop` registration and the read-only awareness ES client that existed only for the selector — `ELASTICSEARCH_URL` is still read for app-log/audit). Tests: gateway 816 → 757 (80 → 75 files), gtd 148 → 118 (9 → 6 files), both green; `tsc --noEmit` clean in both. **Stays:** GTD columns `conversation_id`/`stakes`/`reviewed_at`/`pending_confirm` and migrations 003/004/005, gateway migration 038 (`tray_items.loop_id`), the `ll5_reconcile_metrics` index data, `ll5_eval_moments.close_count` + `eval_record.py`, `query_im_messages`. **Left deliberately (cross-repo contracts, agent PR to retire in lockstep):** the `reconcile` model slot in `agent.ts`/`agent-models.ts` (DB column `model_overrides`, dashboard dropdown, `agent.test.ts` asserts the slot list), the `reconcile-loop` session type in `/internal/agent-session` `validTypes` + migrations 039/040 (the still-running agent posts it until re-provisioned), and `packages/ll5-run-shared/mcp-endpoints.json` `workers.reconcile` (agent-repo config, out of scope). The agent-repo half (`reconcile-loop.sh`, `.mcp.reconcile.json`, `scripts/test_reconcile_*.py`, `docker-entrypoint.sh` launch) is a separate PR; until it lands the worker still calls `list_reconcile_work` and gets a tool-not-found error each tick.
## 2026-09-04 — Agent baseline re-measure is one command (`scripts/agent-baseline.sh`)

`scripts/agent-baseline.sh [--since D] [--until D] [--user U] > docs/reviews/<date>/agent-baseline.md` reproduces the frozen `docs/reviews/2026-09-04/agent-baseline.md` table (chat, rituals, eval moments, mismatch shape, grounding, follow-through, durable knowledge with `note_observation`/day, narratives, journal, session-restart entries/day, `CONSOLIDATE-TALLY` sums, tool calls + housekeeping share + top 10, turn costs, session record, GTD) and ends with a "Delta vs frozen baseline" table (frozen → now, matched on the Area label). Transport is the reviewed pattern: the request list is shipped base64 into the awareness container and run there by `scripts/agent-baseline-esq.js` against its `ELASTICSEARCH_URL` (explicit Basic-auth header; creds never leave the box); GTD via `psql` in the postgres container; all logic in `scripts/agent-baseline.py`. Defaults: `--until` today UTC, `--since` 7 days earlier, inclusive days. Verified against the frozen window (`--since 2026-08-21 --until 2026-09-04`): 60 user msgs (58 + 2 today), 2,339 moments (2,306 + today's), 27,377 tool calls / 20,872 housekeeping (27,055 / 20,700 + today's), rituals 15/15/15, `close_count` 4 / `pencil_count` 14, narratives 219/4/5, `upsert_fact` 6 / `upsert_person` 10 — all drift is same-day activity. Two deliberate differences: GTD overdue uses the gtd MCP's own `due_date < CURRENT_DATE` (61; the frozen 62 counted a due-today item), and the frozen "60 habit outcomes" was a typo (its parts sum to 59). Spill files are not visible from ES — the row says "check inside the container".
