...
│
├── packages/findhub-poller/           # Python sidecar (the stack's only Python) — polls Google Find Hub via GoogleFindMyTools (pinned d46e952), POSTs `tracked_device` items to gateway /webhook. poller.py (loop + gateway POST with explicit UA — Cloudflare 403s default Python UA), findhub_client.py (ISOLATED upstream adapter — drives request_device_list/get_canonic_ids + FcmReceiver/create_location_request/nova_request WITH a timeout + a vendored decrypt loop that RETURNS locations; per-report decrypt is defensive re upstream #22 phone InvalidTag; verified live 2026-06-01, 5 trackers OK), Dockerfile (clones GoogleFindMyTools, Chrome-less — Auth/secrets.json mounted in), README (TWO-login Chrome auth procedure). See docs/design/findhub.md + DECISION-008.
│
