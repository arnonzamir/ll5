...
│       ├── processors/                # GPS geocoding + place/region state machine (location.ts: tracks current semantic label = known place ≤100m OR geocoded city in ll5_awareness_location_state doc per user; FCM push "You're home"/"You're in {city}" only on a label transition, not on distance; replaced the old >200m heuristic), IM processing (message.ts: phone-mirrored SMS/Slack/email — resolves+enriches sender in messaging_contacts by platform=app, synthesizes conversation_id, emits "[Slack] Alice in #x: …" with source routing incl. person_id, WhatsApp-parity), calendar (dedup + enrich), WhatsApp webhook (images, fromMe, pushName enrichment, group participant enrichment, LID→phone mapping; resolves conversation peer display_name + person_id and surfaces them in source routing — outbound renders "You → {recipient}", source carries contact_name/person_id/from_me), WhatsApp contact webhook (CONTACTS_UPSERT/UPDATE), routing rule matcher, phone contacts enrichment
