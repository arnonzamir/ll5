...
│       ├── location/                 # CANONICAL location-resolution domain (2026-06-04, DECISION-009): constants.ts (every location threshold — single source), geo.ts (haversineMeters), types.ts (GpsSignal/WifiSignal/BssidPlace/PriorLabel/ResolvedLocation), filter.ts (gateAccuracy + detectDriftGlitch), resolve.ts (resolveLocation = 7-tier GPS+WiFi fusion + WiFi anchoring + departure hysteresis). Imported by BOTH gateway write/transition path and awareness read path so they can't disagree. Tested in __tests__/location-resolve.test.ts.
