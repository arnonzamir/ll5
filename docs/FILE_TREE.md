...
│       ├── scheduler/tickler-alert.ts   # Polls LL5 ticklers due ≤2h; routes by `kind`: `reminder`→`[Tickler Alert]` (user-facing), `instruction`→`[Agent Instruction]` per-item (agent-private review, no push, full description). Active-hours gated on effective tz.
