...
│   └── docker-compose.prod.yml       # **Production source of truth** (11 services; dashboard at ll5.noninoni.click; google + messaging need AUTH_SECRET in their env block; `browser` = Playwright MCP at mcp-browser.noninoni.click behind Traefik basicAuth + --allowed-hosts, DECISION-010). CI scp's to host on every deploy; never edit on host.
│
├── docs/
...
│       ├── location/                 # CANONICAL location-resolution domain (2026-06-04, DECISION-009): constants.ts (every location threshold — single source; incl. WIFI_CONNECTED_ANCHOR_MS=2h + DEPARTURE_ACCURACY_M=50m anti-flap knobs), geo.ts (haversineMeters), types.ts (GpsSignal/WifiSignal/BssidPlace/PriorLabel/ResolvedLocation), filter.ts (gateAccuracy + detectDriftGlitch), resolve.ts (resolveLocation = 7-tier GPS+WiFi fusion + WiFi anchoring + departure hysteresis). Imported by BOTH gateway write/transition path and awareness read path so they can't disagree. Tested in __tests__/location-resolve.test.ts.
...
│       ├── services/                    # LocationService — fuses GPS + wifi BSSID → CurrentLocation with provenance (used by get_current_location + where_is_user tools); stay-point-service.ts — PURE clustering (no I/O): `detectStayPoints(points, params)` runs classic dwell detection (STAY_RADIUS_M=150, MIN_DWELL_MS=10min, MAX_GAP_MS=30min; all overridable) → `Visit[]` with centroid/start/end/duration/point_count + dominant matched place; `groupVisitsIntoCandidates` groups nearby visit centroids into frequent-place candidates. Backs query_visits + suggest_frequent_places.
