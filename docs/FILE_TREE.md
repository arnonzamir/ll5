...
│
├── packages/findhub-poller/           # Python sidecar (the stack's only Python) — polls Google Find Hub via GoogleFindMyTools, POSTs `tracked_device` items to gateway /webhook. poller.py (loop + gateway POST with explicit UA — Cloudflare 403s default Python UA), findhub_client.py (ISOLATED upstream adapter: list_devices + get_location_data_for_device, defensive normalize, skips unparseable fixes loudly), Dockerfile (clones GoogleFindMyTools, Chrome-less — Auth/secrets.json mounted in), README (one-time Chrome auth procedure). NOT deployed. See docs/design/findhub.md + DECISION-008.
│
