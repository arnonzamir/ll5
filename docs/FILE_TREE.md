...
│       ├── tools/                       # 24 MCP tools (situation, location+delete+where_is_user+query_visits+suggest_frequent_places, messages, journal, user model+versioning, geo search, media, phone_status x2, wifi x2). location.ts now takes the ES client (threaded via tools/index.ts) for the known-place geo_distance exclusion in suggest_frequent_places.
