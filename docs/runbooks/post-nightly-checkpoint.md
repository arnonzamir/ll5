...
5. Workers on Sonnet 5: `~/.ll5/narrative-loop.log` shows at least one completed run (`CONSOLIDATED: …`) with `model=claude-sonnet-5` — the 01:33Z first tick after the roll was `nothing due`, so the model was not exercised yet; if every tick is `nothing due` for the whole day, that is ISS-002/003 (no observations), not the model.
6. ISS-013 fix: no `channel.whatsapp` alert raised overnight (`system_alerts` row `last_seen_at` must predate 2026-09-05 15:00Z or its `status` be resolved with no new `first_seen_at`); gateway logs show `[WhatsAppFlowMonitor]` ticks without `[alert]`.
7. DECISION-030 first night: no agent message to the user between 23:30 and 06:30 local (`ll5_chat_messages` role=assistant, channel web) except level critical; `held_messages` rows released at ~06:30 as one digest (`metadata.kind = quiet_hours_digest`); at least one `NOT SENT` refusal in the transcript followed by a shorter resend (`grep -c "NOT SENT"` on the session jsonl); `GET /me/delivery-mode` answers.
8. Behaviour sanity on Opus 5: `CONSOLIDATE-TALLY` from the 02:00 pass (observations > 0), `note_observation` count for the day, replies to Arnon's messages (latency from `ll5_chat_messages`), no `behavior.*` alert.
9. Write the dated PROGRESS entry + update ISSUES rows; schedule the third run for 2026-09-07 03:12 only if something needed a fix, otherwise the next fixed date is the 7-day readout on 2026-09-12 (`scripts/agent-baseline.sh --since 2026-09-05 --until 2026-09-12`, control `docs/reviews/2026-09-05/agent-baseline-pre-upgrade.md`).
