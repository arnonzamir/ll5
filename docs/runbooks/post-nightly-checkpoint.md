...
7. Behaviour sanity on Opus 5: `CONSOLIDATE-TALLY` from the 02:00 pass (observations > 0), `note_observation` count for the day, replies to Arnon's messages (latency from `ll5_chat_messages`), no `behavior.*` alert.
8. Write the dated PROGRESS entry + update ISSUES rows; schedule the third run for 2026-09-07 03:12 only if something needed a fix, otherwise the next fixed date is the 7-day readout on 2026-09-12 (`scripts/agent-baseline.sh --since 2026-09-05 --until 2026-09-12`, control `docs/reviews/2026-09-05/agent-baseline-pre-upgrade.md`).
