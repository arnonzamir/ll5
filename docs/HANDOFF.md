...
- **PG has no `ADD CONSTRAINT IF NOT EXISTS`** (as of 16). `ALTER TABLE ... ADD CONSTRAINT` at migration time must be guarded by a `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = '...' AND conrelid = 'tbl'::regclass) THEN ... END IF; END $$;` block. The migration runner replays every .sql on every boot; anything non-idempotent silently works until the first replay, then hard-fails startup. Migration 021 hit this Apr 21 and crash-looped the gateway. Also watch for `CREATE FUNCTION` without OR REPLACE, `CREATE TRIGGER` without DROP IF EXISTS, `CREATE TYPE` (no IF NOT EXISTS — wrap in DO block), ALTER TYPE ADD VALUE (same), raw INSERT without ON CONFLICT.
...
- **`/admin/health.system_messages`** — total_failures + recent_by_scheduler + last_error_code from the gateway's `insertSystemMessage` in-memory counter. A non-zero total_failures means the proactive layer is partially dark. Non-admin-visible; use a signed admin token.
