...
- **Authority/Delivery toggle UX (May 3)**: in `/settings/contacts` each toggle button now displays a column-specific label so the same DB value never reads identically across columns: Authority shows Blocked/Read/Reply; Delivery shows Drop/Batch/Notify/Auto. Coupled-bump rule: when both columns are `ignore` and the user clicks one off ignore, the other bumps to its first non-ignore level (permission→input, routing→batch) via a second `upsertContactSetting` call from the Row's onChange.
