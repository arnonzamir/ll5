...
| ISS-016 | scaffolding | med | Nothing monitors session age, compaction cadence, or session-save liveness; sessions roll only on container restart | in-progress | controlled daily restart shipped 2026-09-04 (consolidate hand-off → watcher → fresh session; 26h fallback; 120K context cap; `agent.daily_restart_missing`); verify: one new session/day, zero compactions over 7 days |
