...
| ISS-019 | knowledge | high | Unbounded MCP read results (`read_journal` ~60 KB, `recall_everything` up to 114 KB, one at 1.7 MB) cause the spill | fixed | 2026-09-04: ~20 KB cap + `cursor` on `recall_everything`, `read_journal`, `query_im_messages`, `recall`, `list_narratives`, `get_narrative`, `read_messages`; session transcripts never inlined |
