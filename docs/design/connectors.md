...
|---|---|---|---|
| Phone notification capture | cards, bank, Clalit, municipality, IEC, telecom (event feeds) | none | The phone's ONE generic notification capture (2026-09-08: no per-connector capture on the phone; the gateway routes catalog packages and tells the phone which packages to include via `update_data_source { packages }` / `GET /me/notification-capture`); parsing **server-side** in the gateway (pure parsers, testable, updated without an app release). The phone sends `package, title, text, bigText, postTime`; the gateway keeps raw text only inside `payload_enc` |
...
**Android** (REVISED 2026-09-08, Arnon: no per-connector capture on the phone): the generic notification capture is the only source; `NotificationCaptureService.kt` forwards every package's notifications as `app_notification` items and the gateway routes by package (`processors/app-notification.ts`, one entry point: connector parser + awareness doc with `connector_id` when `connector_<id>` is on, plain awareness doc when off, IM packages dropped). No "Connector capture" screen and no `CONNECTOR_PACKAGES` map in the app. The phone only has to not exclude a connector's app: the `update_data_source` device command for `connector_<id>` carries `packages: [...]` from the catalog (add/drop them in include-list mode), and `GET /me/notification-capture` `{ connector_packages, im_packages }` seeds the include-list on start. The `otp_forwarding_enabled` switch (default off) next to SMS tracking stays as designed.
