...
- **Credential must NOT be visible in `ps` (HARD requirement, found 2026-05-30).** The current single-tenant agent passes its Claude token as a tmux/exec env arg — visible to any process via `ps` inside the container. For multi-tenant hosting the orchestrator MUST inject the user's API key via a **non-process-visible channel**: a `0600` env-file / tmpfs mount read by the entrypoint, a Docker secret, or stdin — never a command-line arg or a plaintext `-e VAR=...` that surfaces in `ps`/inspect. One container per tenant (no shared container), so a co-tenant can never `ps` another's key.
