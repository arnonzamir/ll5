...
| 5 record_moment → sentinel | **Superseded 2026-09-05 (DECISION-030 batch B).** The tool stays: it is now the enforcement point for ISS-004 — `ping_later` is refused without `deferral_ref` (the wake/tickler id booked this turn). A text sentinel cannot refuse. Cost is one local, network-free call per proactive turn (ISS-022 re-rated low). |
