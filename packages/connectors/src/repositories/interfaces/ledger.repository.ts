...
export interface LedgerRepository {
  ...
  /**
   * Rows in [sinceIso, untilIso], in the shape the reconciler takes. User-wide
   * by default (see EventRepository.openForReconcile); pass `connectorId` to narrow.
   */
  forReconcile(sinceIso: string, untilIso: string, connectorId?: string): Promise<ReconcileRow[]>;
  /** Row count for one connector, or for the whole user when `connectorId` is omitted. */
