 ...
 *
 * DECISION-028 #7 (2026-09-04): no longer its own scheduler — TrayItemExpiry runs this
 * function on its 10-minute tick (one sweep, one timer). Same contract as before: dumb. It flips
 ...
 */
export async function expirePermissionRequests(pool: Pool, userId: string): Promise<void> {
  let expired: Array<{
    id: string;
