...
import { PermissionRequestExpiry } from '../scheduler/permission-request-expiry.js';
...
const mk = (pool: Pool) => new PermissionRequestExpiry(pool, { intervalMinutes: 10, userId: USER_ID });
const tick = (s: PermissionRequestExpiry) => (s as unknown as { tick: () => Promise<void> }).tick();
...
describe('PermissionRequestExpiry — authority-request deadline sweep', () => {
